...
HUBSPOT_CLIENT_ID = os.environ.get("HUBSPOT_CLIENT_ID", "your_client_id")
HUBSPOT_CLIENT_SECRET = os.environ.get("HUBSPOT_CLIENT_SECRET", "your_client_secret")
HUBSPOT_REDIRECT_URI = os.environ.get("HUBSPOT_REDIRECT_URI", "http://localhost:8000/hubspot/oauth/callback")
...
async def oauth2callback_hubspot(request: Request):
    ...
    # Extract user_id and org_id from state - assumes format is known
    # In a real implementation, you'd decode this more securely
    ...
    return {"success": True, "message": "HubSpot integration successful"}
...
async def create_integration_item_metadata_object(response_json):
    ...
    # This will depend on the endpoint being called
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            for contact in contacts_data.get("results", []):
                ...
                url = f"https://app.hubspot.com/contacts/contacts/{contact['id']}"
        ...
        if response.status_code == 200:
            ...
            for company in companies_data.get("results", []):
                ...
                url = f"https://app.hubspot.com/contacts/companies/{company['id']}"
